...
// 3. Global CORS Middleware (Fixes client-side "Failed to fetch" errors)
...
// Helper utilities for humanized delays
...
app.get('/api/logs', (req, res) => {
  ...
  res.setHeader('X-Accel-Buffering', 'no'); // Bypass Nginx/Render buffering
  res.flushHeaders();
...
// Periodic ping every 10 seconds to keep Render/Nginx proxies alive
setInterval(() => {
  sseClients.forEach((client) => {
    try {
      client.res.write(': keep-alive\n\n');
    } catch (err) {
      // Cleaned up on connection drop
    }
  });
}, 10000);
...
// 5. Browser Instance Manager with Auto-Reconnect
...
async function getBrowser() {
  ...
  browserInstance.on('disconnected', () => {
    sendLog('Browserless connection dropped. Queueing reconnect...', 'error');
...
// 6. Main Automation Runner Route
app.post('/api/start', upload.single('csvFile'), async (req, res) => {
  try {
    ...
    // Respond immediately to release the client's HTTP request
    ...
    // Run execution in non-blocking background loop
    (async () => {
      for (let i = 0; i < accountRows.length; i++) {
        ...
        while (!success && attempt <= maxRetries) {
          ...
          try {
            ...
            sendLog(`Clicked 'Finish & continue'. Pausing 15s to stabilize new account...`);
            await delay(15000);
        ...
        // Cooldown between rows to prevent server-side rate limits
